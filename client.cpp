...
int main() {
	...
	if (connect(UserSocket, (sockaddr*)&clientADDR, sizeof(clientADDR)) < 0) {
		...
		if (welcome.empty()) {
			std::cout << "Chtenie prervano " << WSAGetLastError() << std::endl;
		}
		else {
		...
		learn.detach();//нужно отвязать поток,объект внури него уничтожается а ждать смерти этог потока не нужно
	}
	while (true) {
		std::string message;
